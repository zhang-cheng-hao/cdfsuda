...
class Meta_model(nn.Module):
    def __init__(self, args):
        ...
        self.num_tar_tasks = args.num_tar_tasks

        self.num_train_tasks = args.num_train_tasks
        self.num_test_tasks = args.num_test_tasks
        self.num_tar_test_tasks = args.num_tar_test_tasks
    ...
    def test(self, support_grads):
        ...
        return accs, roc_list
        
    def final_test(self, support_grads):
        accs = []
        old_params = parameters_to_vector(self.graph_model.parameters())
        for task in range(self.num_tar_test_tasks):
            print(self.num_tar_tasks-task)
            dataset = MoleculeDataset("Original_datasets/" + self.tar_test_dataset + "/new/" + str(self.num_tar_tasks-task), dataset = self.tar_test_dataset)
            support_dataset, query_dataset = sample_test_datasets(dataset, self.tar_test_dataset, self.num_tar_tasks-task-1, self.n_way, self.m_support, self.k_query)
            support_loader = DataLoader(support_dataset, batch_size=self.batch_size, shuffle=False, num_workers = 1)
            query_loader = DataLoader(query_dataset, batch_size=self.batch_size, shuffle=False, num_workers = 1)
