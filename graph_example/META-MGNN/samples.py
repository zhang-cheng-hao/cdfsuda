...
def sample_datasets(data, dataset, task, n_way, m_support, n_query):
    ...
    support_list = sample_inds(range(0,distri_list[task][0]), m_support)
    support_list += sample_inds(range(distri_list[task][0],len(data)), m_support)
    random.shuffle(support_list)

    l = [i for i in range(0, len(data)) if i not in support_list]
    query_list = sample_inds(l, n_query)
