...
class MetaTaskSelector(nn.Module):
    ...
    def compute_prototypes(self, task_data):
        ...
        embeddings = self.graph_embedding(graph_data.x.float(), graph_data.edge_index.long(), graph_data.batch)
        # 使用全局平均池化将节点嵌入聚合为图示例级别的嵌入
        # graph_embeddings = global_mean_pool(embeddings, batch_index)
        # 对于每个类别，计算该类别所有图示例嵌入的平均值，作为类原型
        prototypes = torch.stack([embeddings[graph_labels.long() == i].mean(0) for i in range(self.num_classes)])
