...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
        ...
        if not empty:
            ...
            # 定义文件路径
            file_path = os.path.join(self.processed_dir, 'smiles.csv')
            try:
                # 尝试读取 CSV 文件
                df = pd.read_csv(file_path, header=None)
                self.smiles_list = df.to_numpy()[:, 0]
            except pd.errors.EmptyDataError:
                # 如果文件为空，捕获异常并处理
                print(f"警告: 文件 {file_path} 为空。")
                self.smiles_list = np.array([])  # 赋值为空数组
    ...
    def get(self, idx):
        ...
        if len(self.smiles_list) > 0:
            try:
                ...
                print(f"警告: 索引 {idx} 超出 smiles_list 范围。")
                data.smiles = ""
        else:
            # smiles_list 为空时赋予默认值
            data.smiles = ""
        return data
        ...
        if self.dataset == 'tox21':
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:
                    ...
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
        elif self.dataset == 'BZR' or self.dataset == 'COX2' or self.dataset == 'MUTAG' or self.dataset == 'PROTEINS':
            processed_file = self.raw_paths[0]
            if os.path.exists(processed_file):
                subset = torch.load(processed_file)  # 加载 Subset 对象
                for data in subset:
                    data.y = data.y.long()
