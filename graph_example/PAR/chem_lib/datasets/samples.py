...
def obatin_train_test_tasks(dataset):
    ...
    new_train_tasks = list(range(3))
    new_test_tasks = list(range(3, 4))
    ...
    elif dataset == 'BZR' or dataset == 'COX2' or dataset == 'MUTAG' or dataset == 'PROTEINS':
        return new_train_tasks, new_test_tasks
    else:
        return None, None
...
def obtain_distr_list(dataset):
    if dataset == "sider":
        ...
        return [[74, 27], [77, 24], [82, 19], [86, 16]]
    elif dataset == "COX2":
        return [[84, 32], [91, 25], [91, 25], [99, 20]]
    elif dataset == "MUTAG":
        return [[13, 34], [12, 35], [18, 29], [20, 27]]
    elif dataset == "PROTEINS":
        return [[160, 118], [165, 113], [164, 114], [174, 105]]
    else:
        return None
...
def sample_test_datasets(data, dataset, task, n_shot, n_query, update_step=1):
    """
    将数据集划分为支持集(support set)和查询集(query set)，用于测试阶段的采样。
    Args:
        data: torch.Tensor, 完整的数据集
        dataset: str, 数据集名称
        task: int, 当前任务的索引
        n_shot: int, 支持集中每个类别的样本数量
        n_query: int, 查询集中需要采样的样本数量
        update_step: int, 默认为1，用于调整查询集采样数量的倍数
    Returns:
        tuple: 包含以下三个元素:
            - s_data (torch.Tensor): 支持集数据
            - q_data (torch.Tensor): 完整的查询集数据
            - q_data_adapt (torch.Tensor): 从查询集中采样的部分数据
    Note:
        该函数首先根据阈值将数据分为正负两类，从每类中采样n_shot个样本作为支持集。
        剩余的样本作为查询集，并从中随机采样update_step * n_query个样本作为适应数据。
    """
    distri_list = obtain_distr_list(dataset)
    thresh = distri_list[task][0]
